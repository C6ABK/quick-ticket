...
export async function createTicket(prevState: {success: boolean; message: string}, formData: FormData): Promise<{success: boolean; message: string}> {
    try {
        ...
        if(!user) {
            logEvent('Unauthorised ticket creation attempt', 'ticket', {}, 'warning')
            return {
                success: false,
...
export async function getTickeyById(id: string) {
    try {
...
export async function closeTicket(prevState: {success: boolean, message: string}, formData: FormData): Promise<{success: boolean, message: string}> {
    ...
    if(!ticket || ticket.userId != user.id) {
        logEvent('Unauthorised ticket close attempt', 'ticket', {ticketId, userId: user.id}, 'warning')

        return {success: false, message: 'You are not authorised to close this ticket'}
    }
